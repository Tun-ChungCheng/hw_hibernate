...
@WebServlet("/lab01/updateCustomer.do")
public class CustomerUpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(modify.equalsIgnoreCase("DELETE")){
			...
			String url = request.getContextPath() + "/lab01/queryCustomer.do";
			String newurl = response.encodeRedirectURL(url);
			response.sendRedirect(newurl);
			
		}else if(modify.equalsIgnoreCase("UPDATE")){
			// 讀取瀏覽器送來的 customerId 欄位內的資料
			...
			CustomerBean mb = new CustomerBean(customerId, name, password, phone, date, null, dWeight);
			mb.setId(id);
			ms.update(mb);
